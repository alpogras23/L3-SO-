...
def main():
    ...
    inference = VFAPMAInference(model, device, pixel_spacing_mm=args.pixel_spacing_mm)
    ...
    for i in tqdm(range(len(dataset)), desc="Inference"):
        ...
        try:
            results = inference.infer_single(hu_slice)
            
            # Validate
            is_valid, msg = inference.validate_measurements(
                results['pma_mm2'],
                results['vat_mm2'],
                results['sat_mm2']
            )
            ...
            output_data = {
                ...
                'hu_stats': inference.analyzer.get_tissue_statistics(hu_slice),
                'metadata': metadata
            }
    ...
    if valid_results:
        ...
        logger.info("No valid results to summarize (all cases errored).")
